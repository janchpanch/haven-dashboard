/* 1️⃣  Import the JSON file as a module.  
   The “with { type: 'json' }” syntax tells TS to treat it as a typed JSON import. */
import data from '../data/dataset.json' with { type: 'json' };

/* ----------------------------------------------------------- */
/* 2️⃣  Small “type” definitions – just the bits we need     */
...
type OrderItem = {
  ...
  unit_price_cents: number;  // price of ONE unit (in cents)
};
...
/* ----------------------------------------------------------- */
/* 3️⃣  Build a quick lookup table – id → {name, venue_id}    */

/* Map<string, value> gives us O(1) look‑ups.  The key is the menu_item_id */
const menuLookup = new Map<string, { name: string; venue_id: string }>();
...
for (const mi of data.menu_items as MenuItem[]) {
  ...
  menuLookup.set(mi.menu_item_id, { name: mi.name, venue_id: mi.venue_id });
}

/* ----------------------------------------------------------- */
/* 4️⃣  Types that describe our *aggregated* sales results */
...
/* ----------------------------------------------------------- */
/* 5️⃣  The core helper – walk the orders and accumulate totals */
export function getMenuSales(): SalesByItem {
  ...
  for (const order of data.orders as Order[]) {
    ...
    for (const it of order.items) {
      ...
      const revenueCents = it.unit_price_cents * it.qty;
      ...
      if (!venueData) {
        ...
        venueData = { qty: 0, revenueCents: 0 };
...
Object.values(menuSales).forEach(s => {
  console.log(`${s.name}: ${s.totalQty} sold`);
});
